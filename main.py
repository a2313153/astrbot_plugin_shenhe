...
async def _joingroup(bot: Bot, event: GroupRequestEvent):
    ...
    # 安全获取群号，避免null错误
    group_id = str(event.group_id) if event.group_id else "未知群号"
    user_qq = str(event.user_id) if event.user_id else "未知用户"
...
async def _get_all_group_members(bot: Bot, event: Event):
    ...
    try:
        ...
        for i, group in enumerate(group_list, 1):
            # 安全获取群信息，避免null错误
            group_id = str(group.get('group_id', '未知群号'))
