...
# 初始化插件
plugin = Plugin("main", "群管理插件（带卡密验证功能）")
...
async def handle_join_group(bot: Bot, event: GroupRequestEvent):
    ...
    if not event.group_id or not event.user_id:
        plugin.logger.error("群号或用户ID为空")
        return
        
    group_id = str(event.group_id)
    user_qq = str(event.user_id)
    comment = event.comment or ""  # 确保comment不为None
    
    # 从备注中提取卡密
    def extract_key(comment):
        if not comment:  # 检查comment是否为空
            return ""
    ...
    try:
        ...
        if result.get('status') == 'success' and result.get('usable') == 1:
            ...
            # 检查flag是否存在
            if not event.flag:
                plugin.logger.error("加群请求flag为空，无法处理")
                return
            ...
            plugin.logger.info(f"标记卡密API响应: {mark_response.text if mark_response else '无响应'}")
            
        else:
            # 卡密无效，拒绝加群
            error_msg = result.get('message', '卡密错误') if result else '未知错误'
...
async def get_group_members(bot: Bot, event: Event):
    ...
    if not bot or not event:
        plugin.logger.error("机器人对象或事件为空")
        return
    
    # 权限检查
    user_id = event.get_user_id()
    if not user_id:
        plugin.logger.error("无法获取用户ID")
        return
    ...
    # 提取群号
    cmd_text = event.get_plaintext().strip() if event.get_plaintext() else ""
    ...
    try:
        ...
        for i, group in enumerate(group_list, 1):
            ...
            if not group or 'group_id' not in group:
                plugin.logger.warning(f"无效的群信息：{group}")
                continue
    ...
    try:
        ...
        while True:
            ...
            if isinstance(members, list):
                ...
                if 'data' in members and isinstance(members['data'], list):
        ...
        for m in all_members:
            ...
            processed_members.append({
                "group_id": group_id,
                "user_id": m.get("user_id", ""),  # 确保user_id存在
